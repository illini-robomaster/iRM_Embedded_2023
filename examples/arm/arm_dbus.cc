...
void RM_RTOS_Default_Task(const void* args) {
    ...
    while (true) {
      ...
      osDelay(100);
      if (sbus->ch[6] < 0){
        target_state = {0.0, (float)(sbus->ch[0] / sbus->CHANNEL_MAX), (float)(sbus->ch[1] / sbus->CHANNEL_MAX), (float)(sbus->ch[2] / sbus->CHANNEL_MAX),0.0,0.0,0.0};
...
static const float A1_Kp = 0.002;
static const float A1_Kd = 0.03;
