...
//==================================================================================================
// MotorDMJ10010 (DAMIAO DM-J10010L-2EC)
//==================================================================================================

static void can_motor_dmj10010_callback(const uint8_t data[], void* args) {
  MotorDMJ10010* motor = reinterpret_cast<MotorDMJ10010*>(args);
  motor->UpdateData(data);
}

MotorDMJ10010::MotorDMJ10010(bsp::CAN* can, uint16_t rx_id, uint16_t tx_id, mode_t mode)
    : can_(can), rx_id_(rx_id), tx_id_(tx_id), mode_(mode) {
  can->RegisterRxCallback(rx_id, can_motor_dmj10010_callback, this);
  if (mode == MIT) {
    tx_id_actual_ = tx_id;
  } else if (mode == POS_VEL) {
    tx_id_actual_ = tx_id + 0x100;
...
void MotorDMJ10010::MotorEnable() {
...
void MotorDMJ10010::MotorDisable() {
  uint8_t data[8] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfd};
  this->can_->Transmit(this->tx_id_actual_, data, 8);
}

void MotorDMJ10010::SetZeroPos() {
  uint8_t data[8] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe};
  this->can_->Transmit(this->tx_id_actual_, data, 8);
}

void MotorDMJ10010::SetOutput(float position, float velocity, float kp, float kd, float torque) {
  pos_set_    = position;
  vel_set_    = velocity;
  kp_set_     = kp;
  kd_set_     = kd;
...
void MotorDMJ10010::SetOutput(float position, float velocity) {
  pos_set_ = position;
  vel_set_ = velocity;
}

void MotorDMJ10010::SetOutput(float velocity) {
  vel_set_ = velocity;
}

void MotorDMJ10010::SetOutput(float position, float velocity_limit, float current_limit) {
  pos_set_ = position;
...
mode_t MotorDMJ10010::GetMode() const { return mode_; }

void MotorDMJ10010::TransmitOutput(MotorDMJ10010* motors[], uint8_t num_motors) {
  for (uint8_t i = 0; i < num_motors; ++i) {
    uint8_t data[8] = {0};
...
void MotorDMJ10010::UpdateData(const uint8_t data[]) {
  // Feedback frame (same layout as DM4310 / DM3519):
  //   D[0]: ID | ERR<<4
...
void MotorDMJ10010::PrintData() const {
  set_cursor(0, 0);
  clear_screen();
  print("DMJ10010 Pos: %.4f rad  Vel: %.4f rad/s  Torque: %.4f Nm\r\n",
        GetTheta(), GetOmega(), GetTorque());
  print("         MOS: %d C  Rotor: %d C\r\n", raw_mosTemp_, raw_motorTemp_);
}

float MotorDMJ10010::GetTheta()          const { return theta_; }
float MotorDMJ10010::GetOmega()          const { return omega_; }
float MotorDMJ10010::GetTorque()         const { return torque_; }
