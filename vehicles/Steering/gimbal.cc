...
void gimbalTask(void* arg) {
  ...
  while (true) {
    ...
    if (dbus->keyboard.bit.F || dbus->swl == remote::UP || dbus->keyboard.bit.CTRL) {
      ...
      //set constant to adjust the impact of jetson_yaw on the motor
      //although yaw and pitch from jetson are named 'abs_pitch_jetson',
      //they are actually relative yaw and pitch here.
      double constant = 0.1
      pitch_ratio = dbus->mouse.y / 32767.0 + constant*abs_pitch_jetson;
      yaw_ratio = -dbus->mouse.x / 32767.0 + constant*abs_yaw_jetson;

      pitch_ratio += -dbus->ch3 / 660.0 / 210.0 + constant*abs_pitch_jetson;
      yaw_ratio += -dbus->ch2 / 660.0 / 210.0 + constant*abs_yaw_jetson;
